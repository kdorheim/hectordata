...
test_that("helper function that reads tests/testthat/hector_test_rcp45.ini", {
  test_ini <- read_test_ini()
  lbl <- paste0("Parsed Hector RCP45 test ini file having length > 0")
  expect_true(length(test_ini) > 0, label=lbl)
})
